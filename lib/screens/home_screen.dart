...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void _filter(String query) {
    ...
    setState(() { _visibleSongs = q.isEmpty ? base : base.where((f) => fileName(f.path).toLowerCase().contains(q)).toList(); });
  }
  ...
  void _setView(String view) {
    setState(() { _view = view; if (view != 'PlaylistDetail') _playlistDetail = null; _search.clear(); });
    _filter('');
  }

  Future<void> _toggleFavorite(String path) async {
    setState(() { if (_favorites.contains(path)) _favorites.remove(path); else _favorites.add(path); });
    await _save();
    _filter(_search.text);
  }
  ...
  Future<void> _addFolder() async {
    ...
    try {
      ...
      setState(() { _folders.add({'name': path.split(Platform.pathSeparator).last, 'path': path, 'enabled': true}); });
      await _save();
      await _scan();
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _createPlaylist() async {
    ...
    setState(() { _playlists.add({'name': name, 'songs': <String>[]}); });
    await _save();
    _setView('Playlists');
  }

  Future<void> _addToPlaylist(File song) async {
    if (_playlists.isEmpty) { await _createPlaylist(); if (_playlists.isEmpty) return; }
    final selected = await showDialog<int>(context: context, builder: (context) => SimpleDialog(
      title: const Text('Add to Playlist'),
      children: List.generate(_playlists.length, (i) => SimpleDialogOption(onPressed: () => Navigator.pop(context, i), child: Text(_playlists[i]['name'] as String))),
    ));
        ...
        builder: (context, setModal) {
          return Container(
            ...
            decoration: const BoxDecoration(
              gradient: LinearGradient(colors: [Color(0xFF0F172A), Color(0xFF1E1B4B)], begin: Alignment.topCenter, end: Alignment.bottomCenter),
              ...
              children: [
                ...
                Container(
                  ...
                  decoration: const BoxDecoration(
                    ...
                    gradient: LinearGradient(colors: [Color(0xFF4C83FF), Color(0xFFD946EF)]),
                  ),
                ...
                Row(
                  ...
                  children: [
                    ...
                    IconButton(iconSize: 28, onPressed: () => _addToPlaylist(_visibleSongs[_currentIndex]), icon: const Icon(Icons.playlist_add, color: Colors.white70)),
                  ],
                ),
          ...
          child: Column(
            children: [
              Container(
                ...
                decoration: const BoxDecoration(
                  gradient: LinearGradient(colors: [Color(0xFF1E1B4B), Color(0xFFD946EF)]),
                ),
                child: const Column(
